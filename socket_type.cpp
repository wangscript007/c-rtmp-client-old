...
bool g_bOpen = false;
bool g_bClose = false;
FILE *fpMetaData = nullptr;
const char *metaName = "meta.data";
...
socket_t socketCreateTcpClient(const char* hostname,int port)
{
	...
	do 
	{
		...
		int iRet = getaddrinfo(hostname, portStr, &hints, &result);
		if (iRet)
		{
			break;
		}
...
socket_t socketCreateServer(const char* hostname,int port)
{
	...
	do 
	{
		...
		if (iRet!=0)
		{
			printf("Set socket %d no blocking failed;",svrSocket);
			break;
		}
...
{
	int ret = 0;
	if (len==0||s<0||buf==nullptr)
	{
		ret = -1;
		return ret;
	}
	int recved = 0;
	if (g_bOpen&&!fpMetaData)
	{
		fpMetaData = fopen(metaName, "wb");
	}
	if (g_bClose&&fpMetaData)
	{
		fclose(fpMetaData);
		fpMetaData = nullptr;
	}
	while (recved<len)
	{
		ret = recv(s, buf + recved, len - recved, 0);
		if (ret<=0)
		{
			ret = socketError();
	...
	if (len>0&&g_bOpen&&fpMetaData)
	{
		fwrite(buf, len, 1, fpMetaData);
	}
	return len;
